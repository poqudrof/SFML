...
bool SoundFileReaderMp3::check(InputStream& stream)
{
    ...
    // Basic MP3 format detection by checking for the ID3 tag and a valid frame header
    if (std::memcmp(header, "ID3", 3) == 0 && !((header[5] & 15) || (header[6] & 0x80) || (header[7] & 0x80) || (header[8] & 0x80) || (header[9] & 0x80)))
        return true;
    if (hdr_valid(header))
        return true;
